...
## 🎯 主题范围(WeWrite 核心宗旨 · 2026-04-26 重锁)

**所有文章必须命中以下 7 个主题之一 · 越界即重写**:

1. **AI 干货** — 实用 know-how / 框架 / 模板 / 直接能复用的方法论(不是新闻、不是综述)
2. **AI 教程** — Step-by-step 上手 / 跑通流程 / 「明天就能用」级别
3. **AI 赚钱** — 副业 / 变现 / 月入 / 路径 / ROI / 红利窗口
4. **AI 创业** — 创业实战 / 0→1 / 业务模式 / 团队 / 融资 · 围绕 AI 产品业务
5. **AI 真实测评** — 工具实测 / 横向对比 / 数据 / 必须有不利发现 · 不是软广
6. **AI 踩坑** — 失败案例 / 翻车经过 / 教训复盘 / 反直觉的坑
7. **AI 感悟** — 反共识 / 冷观察 / 工程派判断 / 长期视角 · 不堆 hype

**🚫 明确不做**(2026-04-26 用户原话「ai 新闻咨询不制作这类内容」):
- ❌ **AI 新闻资讯**(模型发布速报 / 行业新闻播报 / 资金消息)
- ❌ **AI 行业咨询**(投资视角 / 趋势分析 / 宏观判断)
- ❌ **普通副业**(读书 / 写作 / 自媒体涨粉)非 AI 副业
- ❌ 政治 / 股票币圈 / 同行八卦 / 个人感情(`identity/voice/forbidden.md`)
- ❌ 翻译式无价值复述 / 「AI 让生活更美好」式空话
- ❌ 健康 / 教育 / 旅游 / 养生 / 娱乐 / 写作鸡汤(`粥左罗式`)

**为什么这 7 个**:用户智辰人设「哈工大本硕 + 6 年半导体 → AI 创业实战派」·
偏**干货 + 实战 + 真实数据 + 踩坑反思** · 跟新闻播报/行业咨询是两条赛道。
4/26 用户重锁(旧版 6 大主题 4/25 太宽 · 导致粥左罗写作鸡汤霸榜 idea_bank)。

**强制守门(代码层 · 不靠 prompt 自觉)**:
- `auto_pick.py` 必须 `_is_ai_topic()` 关键词守门 + `topic-curator` sub-agent 双保险
- `fetch_kol.py` 文章入库前按标题/正文 AI 关键词过滤(非 AI 入 archive)
- `write.py` 4 个 prompt builder 在尾部注入这 7 主题清单
- `auto_review.py` 自审加「主题命中范围」维度
- `kol_list.yaml` 招 KOL 必须 tags 含 AI / AIGC / 大模型 / Agent / AI 评测
