...
_HUMANIZE_RULES = (
    ...
    "11. **★ 开头必须反共识**(不要 LLM 套路化『最近...火了』开头):\n"
    "    ✅「90% 的人在这事儿上花了冤枉钱。」「这事儿一夜变了。」「不是 X · 是 Y。」\n"
    "    ❌「最近 AI 圈又有大新闻」「随着技术发展」「众所周知」\n"
    "12. **★ 技术术语口语化注解**(LLM 写完整专业名 · 人写注解):\n"
    "    ✅「ARR(年度经常性收入,通俗说就是订阅一年能收多少钱)」「FT(微调,简单说就是给模型补习)」\n"
    "    ❌ 整篇都是 ARR / FT / RAG / Agent 不解释\n"
    "13. **★ 至少 1 处『失败 / 踩坑 / 不确定』披露**(强可信度信号):\n"
    "    ✅「我上个月把 onnx 1.16 升 1.18 · 报错 3 小时才搞定」「这个数我没完全测准 · 偏差 ±15%」\n"
    "    ❌ 全文都是成功案例 + 完美数字(LLM 倾向)\n"
    "14. **★ 段尾不要 LLM 套话总结**:\n"
    "    禁:「总而言之」「希望对你有帮助」「值得关注」「让我们拭目以待」「未来可期」\n"
    "    改:「这事我先不下结论。」/ 「先按下不表。」/ 「评论区说你怎么看?」\n"
    "15. **★ 偶尔无意义短句**(人发文有 · LLM 没有):\n"
    "    「真的。」「我服了。」「就是这么离谱。」「可怕。」(单句段 · 1 篇 1-2 处)\n\n"
    "**自检步骤**(写完后):\n"
    "- 数一下「然而 / 此外 / 总之 / 综上」出现几次 · ≥1 次重写\n"
    "- 数一下整数(100 / 50% / 1000)出现几次 · ≥3 次改成不规整数字\n"
    "- 看段长方差 · 全 3-5 行段就太规整 · 加几处单句段\n"
    "- 看有没有真实地点/时间/人名 · 0 处必加 1-2 处\n"
    "- 看有没有失败/踩坑/不确定披露 · 0 处加 1 处\n"
    "- 开头 30 字看是否反共识 · 不是就重写第一段\n\n"
)
