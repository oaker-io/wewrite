...
def run_claude_write(topic, date_str, out_path: Path):
    ...
    prompt = (
        ...
        "3. 文末必须带完整 IP 引流段:\n"
        "   - `:::author-card`(smart 自动按 theme 匹配色系)\n"
        "   - 两个二维码 `![](images/qr-zhichen.png)` 和 `![](images/qr-openclaw.png)`\n"
        "   - aipickgold.com 安利段\n"
        "4. H1 标题 20-28 字 · 正文 1800-2500 字 · 至少 2 个编辑锚点 `<!-- ✏️ ... -->`\n"
        "5. H1 下面第一行插入封面占位符 `![封面](images/cover.png)`\n"
        "6. 按需插入内文 chart 占位符 `![](images/chart-1.png)` ... `chart-4.png`\n"
        "7. 写完返回一行:'DONE {absolute_path}'\n\n"
