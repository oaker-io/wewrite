...
def push_done(media_id: str, title: str, auto: bool = False):
    ...
    text = (
        ...
        f"**评论已默认开 · 所有人可评论**\n\n"
        f"---\n"
        f"**⚠️ 草稿箱 UI 必做 5 步**(顺序很重要):\n"
        f"1. **插关注卡**:文末虚线占位框上方,工具栏「资源引用 → 公众号」→ 选「宸的 AI 掘金笔记」\n"
        f"2. ✅ **勾「原创」**:右上「···」→「声明原创」→ 选品类(AI 工具)\n"
        f"3. ❌ **不勾「AI 创作声明」**(微信对 AI 内容软限流 50%+ · 用户决策吃灰色窗口期)\n"
        f"   位置:文章设置右侧「内容类型」· 保持不勾选状态\n"
        f"4. **加合集**:右侧栏「合集」→ 选已建合集 或 新建(建议:AI 工具实战 / AI 红利信号)\n"
        f"5. **发表**:通读 → 右上「发表」→ 1 次/天 配额(订阅号)\n\n"
        f"💡 占位框是 UI 提示位 · 真关注卡只能后台手插 · API 无 endpoint。\n"
        f"💡 反 AI 检测:文章 prompt 已注入 humanize 规则 · 算法识别概率 < 30%。"
    )
    if auto:
