...
## 11. identity 同步(从 xwrite 反向 vendor)

### 11.1 identity 怎么用
wewrite 写文章时,`scripts/workflow/write.py` 自动从 `wewrite/identity/` 读关键 section 注入 prompt:
- `identity.md` 的 ## 0-4 节(身份/定位/bio/IP 故事)
- `voice/catchphrases.md`(招牌句式 · 让 AI 自然带出)
- `voice/forbidden.md`(禁忌词 · AI 必须避免)

注入的内容在 hotspot 和 tutorial 两个 prompt 都生效。

### 11.2 从 xwrite 同步(默认 diff 看变化)
```bash
scripts/sync-identity-from-xwrite.sh
```
**期待**:列出 5 个文件每个的 diff(无变化时显示「✓ 无变化」)。**不真覆盖**。

### 11.3 真同步(覆盖 wewrite/identity/)
```bash
scripts/sync-identity-from-xwrite.sh --apply
```
**期待**:每个文件输出 `✓ <filename>` 或 `· <filename> (unchanged)` · 末尾更新 `.last-synced` 时间戳

### 11.4 检查多久没同步(给 routine 用)
```bash
scripts/sync-identity-from-xwrite.sh --check-staleness
```
**期待**:打印一个数字(N 天)· 999 = 从未同步过

### 11.5 失败排查
- xwrite identity 不存在:`~/xwrite/identity/` 缺(检查路径)
- diff 没显示但 `.last-synced` 旧:文件确实没变 · 不需要同步
- write.py 没注入(prompt 里没「作者人设档案」):`identity/identity.md` 不存在 · 跑一次 `--apply`
